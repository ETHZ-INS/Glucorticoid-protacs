...
We search for cases where an inhibitor or the protac has flipped the sign of a logFC. We filter for genes where the gene was DE in both the comparison with Dex 18h as well as inhibitor/protac. We apply the additional filter than the sign of the logFC has changed with reference to the Dex 18h experiment.

```{r, fig.width=10, fig.height=7, results="asis", warning=FALSE}
...
merged216$category3 <- (merged216$FDR < 0.05) & 
  ((merged216$FDR_MIF > 0.05) | ((merged216$FDR_MIF < 0.05) & (merged216$logFC * merged216$logFC_MIF > 0))) & # 18h DEX and MIF point in same direction
  ((merged216$FDR_cort > 0.05) | ((merged216$FDR_cort < 0.05) & (merged216$logFC * merged216$logFC_cort > 0))) & # 18h DEX and MIF point in same direction
...
Because we have a lot of genes to show in one plot for the 16h > 2h experiment, we split them into two groups. One plot shows the genes for which the Dex 2h logFC is >0 and the other <0.

```{r plots, fig.width=14, fig.height=5, warning=FALSE}
...
We search for cases where an inhibitor or the protac has flipped the sign of a logFC. We filter for genes where the gene was DE in both the comparison with the Dex 2h as well as inhibitor/protac. We apply the additional filter than the sign of the logFC has changed with reference to the Dex 2h experiment.
...
merged162$category3 <- (merged162$FDR < 0.05) & 
  ((merged162$FDR_MIF > 0.05) | ((merged162$FDR_MIF < 0.05) & (merged162$logFC * merged162$logFC_MIF > 0))) & # 2h DEX and MIF point in same direction
  ((merged162$FDR_cort > 0.05) | ((merged162$FDR_cort < 0.05) & (merged162$logFC * merged162$logFC_cort > 0))) & # 2h DEX and MIF point in same direction
