---
title: 'Q1b: DE Gene overlap for 2h data'
...
colData(SE)$Treatment <- c(rep("Control",4), rep("Dex2h", 4))
```
...
saveRDS(file="../data/EOS_files/Q1_SummExp_2h.rds", SE)
```
...
venn <- venn.diagram(
  ...
  filename = NULL, #'../docs/assets/1_venn_diagram_2h.svg',
  # imagetype="svg",
  output=FALSE,
...
ggsave(venn, file='../docs/assets/1_venn_diagram_2h.pdf', device = "pdf")
ggsave(venn, file='../docs/assets/1_venn_diagram_2h.png', device = "png")
```

```{bash, engine.opts='-l'}
cp ../docs/assets/1_venn_diagram_2h.pdf ../docs/00_paper_figures/6a2_venn.pdf
```

![](assets/1_venn_diagram_2h.png)
...
ggsave(file="../docs/assets/2_logFC_Mahshid_Reddy_2h.pdf", plot_lfc)
```
