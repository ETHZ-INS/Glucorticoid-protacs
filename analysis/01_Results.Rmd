...
```{r plot_signals_and_sechm function, fig.width=18, fig.height=10}
...
plot_signals_and_sechm <- function(peaks, genes_subset=NULL, plot_around_gene_tss=FALSE, tracks_subset=NULL, subset_columns=NULL, show_rownames=FALSE, assayName="log2FC", nclusters=NULL){
  ...
  if (plot_around_gene_tss == TRUE){
    ...
    mask <- c(mask1, mask2)
    # only subset if mask has length > 0
    if (length(mask) != 0) mygenes_unique <- mygenes_unique[-mask]
  
    # get the signals
    signals <- signal2Matrix(tracks_used, mygenes_unique, w=5, extend=2500) # 13820
    ...
    # we need a peak to plot
    peaks <- peaks[peaks$nearestTSS.gene_name %in% row.names(se0)]
    peaks <- sort(peaks)
  ...
  obj <- h1 + h2
  draw(obj, padding = unit(c(1, 1, 2, 1), "cm"))
}

plt <- plot_signals_and_sechm(gr_peaks, plot_around_gene_tss=FALSE)
plt
```
...
```{r 6c.1, fig.width=14, fig.height=10, results="asis"}
reduced_view <- plot_signals_and_sechm(gr_peaks, plot_around_gene_tss=TRUE, subset_columns = c("18h untreated", "18h DMSO", "16h DMSO >\n2h DEX+DMSO", "16h KH-103 >\n2h DEX+KH-103", "16h MIF  >\n2h DEX+MIF", "16h Cort113 >\n2h DEX+Cort113"), tracks_subset=c("2h-DEX"), nclusters=3)
reduced_view

pdf("../data/00_paper_figures/6c1.pdf", width=14, height=10)
reduced_view
dev.off()
...
```{r 6d1, fig.width=14, fig.height=12, results="asis", warning=FALSE}
reduced_view2 <- plot_signals_and_sechm(gr_peaks, plot_around_gene_tss=TRUE, 
                                       subset_columns = c("18h untreated", "18h DMSO", "2h DEX >\n16h DEX+DMSO", "2h DEX >\n16h DEX+KH-103", "2h DEX >\n16h DEX+MIF", "2h DEX >\n16h DEX+Cort113"), tracks_subset=c("12h-DEX"), nclusters=3)

reduced_view2

pdf("../data/00_paper_figures/6d1.pdf", width=14, height=10)
reduced_view2
dev.off()
...
```{r 7b2, fig.width=14, fig.height=4, results="asis", warning=FALSE}
kh.protac <- read.xlsx("../docs/downloads/Q4_KH_DE_GR_bound.xlsx")

# Maybe if possible also including the long range enhancer info as well
genes <- kh.protac[kh.protac$FDR < 0.05,]

fig7b <- plot_signals_and_sechm(gr_peaks, genes_subset=genes$Gene, plot_around_gene_tss=TRUE, subset_columns = c("18h untreated", "18h DMSO", "18h KH-103", "2h DEX >\n16h DEX+DMSO"), tracks_subset=c("2h-DEX", "12h-DEX"), show_rownames=TRUE)
...
pdf("../data/00_paper_figures/fig7b2.pdf", width=8, height=4)
fig7b
dev.off()
...
```{r signaltracks plus de plot, fig.width=22, fig.height=10, results="asis"}
...
```{r fig7b.1, fig.width=18, fig.height=14, results="asis"}
...
plt_inhibs_not_kh  <- plot_signals_and_sechm(gr_peaks, genes_subset=rnames_inhibs_not_kh, plot_around_gene_tss=TRUE, subset_columns = c("18h untreated", "18h DMSO", "18h KH-103", "18h MIF", "18h Cort113", "16h DMSO >\n2h DEX+DMSO", "2h DEX >\n16h DEX+DMSO"), tracks_subset=c("12h-DEX"))
plt_inhibs_not_kh

pdf("../data/00_paper_figures/7b1.pdf", width=14, height=14)
plt_inhibs_not_kh
dev.off()
# ggsave(plot=ggplotify::as.ggplot(plt_inhibs_not_kh), filename="../data/00_paper_figures/7b1.pdf", width=10, height=14)
```
...
```{r signal tracks, fig.width=8, fig.height=4}
tracks2 <- list("C2"="../data/single_replicate_peaks/control/Chip_GR_control_s2.bigWig",
...
m <- signal2Matrix(tracks2, merged_peaks)
...
```{r average signal plots, fig.width=8, fig.height=6}
d1 <- meltSignals(m)
...
## Fig 6c.1 and 6d.1 (interaction signals)

```{r 6c1 and 6d1, fig.width=18, fig.height=10, results="asis", warning=FALSE}
# subset genes which do not have a GR peak
# From the heatmap you're already making, extract the genes for which there's no GR binding at the TSS.
# list of all genes
all.genes <- rownames(se)
# which are not closest to a peak
gr.orphans.genes <- all.genes[!all.genes %in% gr_peaks$nearestTSS.gene_name]


tracks.2h12h <-  list("2h-DEX"="../data/reddy_bigwig_DEX2h_r123.bigWig",
                "12h-DEX"="../data/reddy_bigwig_DEX12h_r123.bigWig")

# prefilter for only the relevant genes
interactions.filtered <- interactions %>%
  dplyr::filter(symbol %in% gr.orphans.genes)
interactions.prefiltered.gr <- makeGRangesFromDataFrame(interactions.filtered, keep.extra.columns=T)

if (!file.exists("../data/EOS_files/sm_lris.rds")){
  sm <- signal2Matrix(tracks.2h12h, interactions.prefiltered.gr)  
  saveRDS(sm, "../data/EOS_files/sm_lris.rds")
} else {
  sm <- readRDS("../data/EOS_files/sm_lris.rds")
}

if (!file.exists("../data/EOS_files/sm_lris.rds")){
  interactions.filtered$GR.score <- pmax(enriched_score(sm[[1]]), enriched_score(sm[[2]]))
  saveRDS(interactions.filtered, "../data/EOS_files/interaction_filtered.rds")
} else {
  interactions.filtered <- readRDS("../data/EOS_files/interaction_filtered.rds")
}

# For these genes, find those that have an annotated distal enhancer (using the predictions I shared with you a while back).
interactions.filtered <- interactions.filtered %>% 
  group_by(symbol) %>% 
  top_n(-1, wt=GR.score) %>% 
  mutate("nearestTSS.gene_name"=symbol)

interactions.filtered.granges <- makeGRangesFromDataFrame(interactions.filtered, keep.extra.columns=T)

# For those, do the heatmap with these enhancers instead of the TSS. (If a gene has more than one annotated distal enhancer, use the one with the highest GR signal)

### 6c1
all.genes.enhancer.tracks6c1 <- plot_signals_and_sechm(interactions.filtered.granges, plot_around_gene_tss=FALSE, subset_columns = c("18h untreated", "18h DMSO", "16h DMSO >\n2h DEX+DMSO", "16h KH-103 >\n2h DEX+KH-103", "16h MIF  >\n2h DEX+MIF", "2h DEX >\n16h DEX+Cort113"), tracks_subset=c("2h-DEX", "12h-DEX"), nclusters=3)
all.genes.enhancer.tracks6c1

pdf("../data/00_paper_figures/6c1b.pdf", width=14, height=10)
all.genes.enhancer.tracks6c1
dev.off()


### 6d1
all.genes.enhancer.tracks6d1 <- plot_signals_and_sechm(interactions.filtered.granges, plot_around_gene_tss=FALSE, 
                                       subset_columns = c("18h untreated", "18h DMSO", "2h DEX >\n16h DEX+DMSO", "2h DEX >\n16h DEX+KH-103", "2h DEX >\n16h DEX+MIF", "2h DEX >\n16h DEX+Cort113"), tracks_subset=c("2h-DEX", "12h-DEX"), nclusters=3)

all.genes.enhancer.tracks6d1

pdf("../data/00_paper_figures/6d1b.pdf", width=14, height=10)
all.genes.enhancer.tracks6d1
dev.off()
```
