...
```{r}
peaks <- import.bed15("../data/reddy_bed_DEX2h_r123.bed.gz", format="narrowPeak")
```
...
# column only for peaks with 2500np range
peakAnno$nearestTSS.gene_name.2500r <- peakAnno$nearestTSS.gene_name
peakAnno[abs(peakAnno$distance2nearestTSS) >= 2500, ]$nearestTSS.gene_name.2500r <- ""
...
peakfreq <- as.data.frame(table(peakAnno$nearestTSS.gene_name)) %>% mutate_all(na_if,"") %>%
  left_join(as.data.frame(table(peakAnno$nearestTSS.gene_name.2500r)) %>% mutate_all(na_if,""), by="Var1")
colnames(peakfreq) <- c("Gene", "Freq", "Freq_2500peaks")

saveRDS(file="../data/EOS_files/Q2_gene_peaks_2h.rds", peakfreq)
```
...
# names(res_int_all)
res_int <- res_int_all$`16h DMSO > 2h DEX+DMSO`
...
res_int <- left_join(res_int %>% mutate(Gene = rownames(res_int)), 
          ...
          by="Gene") %>%
  tidyr::replace_na(list(Freq=0, Freq_2500peaks=0)) %>%
  mutate(gc_bound = Freq > 0) %>%
  dplyr::rename(chip_peaks=Freq, chip_peaks.2500r=Freq_2500peaks) %>%
  relocate(Gene)
...
```{r, message=FALSE}
ggsave(file="../docs/assets/Q2_1_pvalue_dist_2h.pdf", plt_value)
ggsave(file="../docs/assets/Q2_2_logfc_dist_2h.pdf", plt_logfc)
ggsave(file="../docs/assets/Q2_3_mahshid_reddy_2h.pdf", combined_plot, width=12, height=8)
```
...
```{r distribution by numbers of peaks, fig.width=14, message=FALSE}
mixed_data <- rbind(as.data.frame(res_reddy) %>% 
        tibble::rownames_to_column("Gene") %>%
        dplyr::select("Gene", "logFC", "chip_peaks", "chip_peaks.2500r") %>%
        mutate(Source="Reddy"),
  ...
  mutate(chip_peaks=factor(chip_peaks, levels=sort(as.numeric(unique(chip_peaks)))),
         chip_peaks.2500r=factor(chip_peaks.2500r, levels=sort(as.numeric(unique(chip_peaks.2500r)))),
         abs_logFC=abs(logFC))
...
boxplot <- ggplot(mixed_data, aes(x=chip_peaks, y=logFC, colour=Source)) +
  geom_boxplot() +
  theme_ipsum_rc() +
  labs(title="Distribution of logFC by number of nearby GR-peaks", subtitle="Closest peak to gene no matter distance", x="# ChIP peaks") +
  ylim(c(-5,5))

boxplot_2500 <- ggplot(mixed_data, aes(x=chip_peaks.2500r, y=logFC, colour=Source)) +
  geom_boxplot() +
  theme_ipsum_rc() +
  labs(title="", subtitle="Peaks within 2500bp range", x="# ChIP peaks") +
  ylim(c(-5,5))

boxplot_abs <- ggplot(mixed_data, aes(x=chip_peaks, y=abs_logFC, colour=Source)) +
  geom_boxplot() +
  theme_ipsum_rc() +
  labs(title="Distribution of ABSOLUTE logFCs by number of nearby GR-peaks", subtitle="Closest peak to gene no matter distances", x="# ChIP peaks") +
  ylim(c(0,5))

boxplot_abs_2500 <- ggplot(mixed_data, aes(x=chip_peaks.2500r, y=abs_logFC, colour=Source)) +
  geom_boxplot() +
  theme_ipsum_rc() +
  labs(title="", subtitle="Peaks within 2500bp range", x="# ChIP peaks") +
  ylim(c(0,5))
...
boxplot <- ggarrange(
  ggarrange(boxplot, boxplot_2500, ncol=2),
  ggarrange(boxplot_abs, boxplot_abs_2500, ncol=2),
  ncol=1)
boxplot

ggsave(filename="../docs/assets/Q2_4_boxplots_2h.pdf", boxplot, width=14, height=10, unit="in")
```


# Relationship between a logFC and the closest GR peak

```{r logFC vs GR peak, fig.width=14, fig.height=18, message=FALSE}
...
min_dist2 <- plt_data %>%
  ggplot(aes(log(min_dist2tss), logFC, size=n_peaks, colour=is_significant)) +
  geom_point(alpha=0.8) +
  geom_density_2d(bins = 10, colour="green") +
  labs(title="LogFC vs log(min distance peak to TSS)", subtitle="Size indicates the number of peaks for a given gene") +
  ylim(c(-3,3))

TSS_distances <- ggarrange(min_dist, min_dist2, ncol=1)
TSS_distances

ggsave(filename="../docs/assets/Q2_5_TSS_distances_2h.pdf", TSS_distances)
```

```{r, fig.width=14, fig.height=18, message=FALSE}
# Min distance to TSS
min_dist_xlim <- ggplot(plt_data, aes(min_dist2tss, logFC, size=n_peaks, colour=is_significant)) +
  geom_point(alpha=0.8) +
  geom_density_2d(bins = 10, colour="green") +
  labs(title="LogFC vs minimum distance peak to TSS", subtitle="Size indicates the number of peaks for a given gene") +
  xlim(c(0,100000))
...
TSS_distances_xlim <- ggarrange(min_dist_xlim, min_dist2_xlim, ncol=1)
TSS_distances_xlim

ggsave(filename="../docs/assets/Q2_5_TSS_distances_xlim2500_2h.pdf", TSS_distances_xlim)
ggsave(filename="../docs/00_paper_figures/6b_2h.pdf", TSS_distances_xlim)
```


```{r boxplots based on class, fig.width=14, message=FALSE}
...
ggsave(filename="../docs/assets/Q2_6_boxplots_class_2h.pdf", group_image2, width=14, height=10, unit="in")
```
